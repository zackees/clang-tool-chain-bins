...
pub fn split_archive(archive_path: &Path, max_part_size_mb: u64, output_dir: &Path) -> Result<Vec<PartInfo>> {
    let file_size = fs::metadata(archive_path)?.len();
    let max_bytes = max_part_size_mb * 1024 * 1024;

    if file_size <= max_bytes {
        println!("Archive is {} — no splitting needed (limit: {} MB)",
            ctcb_core::format_size(file_size), max_part_size_mb);
        return Ok(vec![]);
    }

    fs::create_dir_all(output_dir)?;

    let stem = archive_path.file_name()
        .context("No filename")?
        .to_string_lossy();
    ...
    loop {
        ...
        println!("  Part {}: {} (sha256: {}...)", part_num, ctcb_core::format_size(part_bytes), &sha256[..16]);
        ...
        part_num += 1;
        if part_bytes < max_bytes { break; } // Last chunk was smaller = EOF
    }
