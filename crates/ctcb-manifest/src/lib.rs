...
pub const LFS_MEDIA_BASE: &str = "https://media.githubusercontent.com/media/zackees/clang-tool-chain-bins";
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        for (key, value) in obj {
            ...
            let info: VersionInfo = serde_json::from_value(value.clone())
                .map_err(|e| serde::de::Error::custom(format!("bad version entry '{}': {}", key, e)))?;
...
pub fn update_platform_manifest(
    ...
    manifest.versions.insert(version.to_string(), VersionInfo {
        href: href.to_string(),
...
pub fn ensure_root_entry(
    ...
    let mut root = if path.exists() {
        ...
        RootManifest { platforms: Vec::new() }
    };

    // Find or create platform entry
    let platform_entry = root.platforms.iter_mut()
...
mod tests {
    ...
    fn test_platform_manifest_roundtrip() {
        let manifest = PlatformManifest {
            ...
            versions: {
                ...
                m.insert("21.1.5".to_string(), VersionInfo {
  ...
  "21.1.5": {
    ...
    "sha256": "3c21e45edeee591fe8ead5427d25b62ddb26c409575b41db03d6777c77bba44f"
        ...
        assert!(manifest.versions["21.1.5"].href.contains("media.githubusercontent.com"));
    }
    ...
    fn test_update_platform_manifest_creates_new() {
        ...
        update_platform_manifest(&path, "21.1.5", "https://example.com/a.tar.zst", "abc123", None).unwrap();
    ...
    fn test_update_platform_manifest_adds_version() {
        ...
        update_platform_manifest(&path, "21.1.5", "https://example.com/a.tar.zst", "abc123", None).unwrap();
        update_platform_manifest(&path, "21.1.6", "https://example.com/b.tar.zst", "def456", None).unwrap();
    ...
    fn test_lfs_media_url() {
        ...
        assert_eq!(url, "https://media.githubusercontent.com/media/zackees/clang-tool-chain-bins/main/assets/clang/win/x86_64/llvm-21.1.5-win-x86_64.tar.zst");
    }
                ...
                for arch in &platform.architectures {
                    ...
                    if platform_path.exists() {
                        ...
                        for info in pm.versions.values() {
                            assert!(info.href.contains("media.githubusercontent.com") || info.href.contains("localhost"),
                                "URL should use LFS media format: {}", info.href);
                        }
