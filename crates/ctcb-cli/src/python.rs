...
fn expand_archive(archive: &str, output_dir: &str, verify: Option<&str>) -> PyResult<()> {
    ...
    std::fs::create_dir_all(&output_dir).map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
...
fn read_platform_manifest(path: &str) -> PyResult<String> {
    let manifest = ctcb_manifest::read_platform_manifest(&PathBuf::from(path))
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
    serde_json::to_string_pretty(&manifest).map_err(|e| PyRuntimeError::new_err(e.to_string()))
}
...
fn update_platform_manifest(
    ...
    ctcb_manifest::update_platform_manifest(&PathBuf::from(path), version, href, sha256, part_refs)
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))
}
