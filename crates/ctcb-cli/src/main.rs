...
fn discover_platform_arch_dirs(
    ...
    if combos.is_empty() {
        anyhow::bail!("No platform/arch directories found in {}", root.display());
    }
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Expand {
            ...
            ctcb_archive::extract_tar_zst(&archive, &output_dir).context("extraction failed")?;
        ...
        Commands::Dedup { action } => match action {
            ...
            DedupAction::Create {
                ...
                let manifest = ctcb_dedup::create_deduped_structure(&source_dir, &dest_dir)?;
        ...
        Commands::Download {
            ...
            ctcb_download::download_all(&version, platforms.as_deref(), &output, !no_verify)?;

            Ok(())
        }
        Commands::BenchCompression {
            directory,
            output_prefix,
        } => {
        ...
        Commands::Fetch {
            ...
            let output = output_dir
                .unwrap_or_else(|| PathBuf::from(format!("assets/clang/{}/{}", plat, ar)));
            ...
            let strip_stats = ctcb_strip::strip_llvm(&extracted_dir, &stripped_dir, &strip_config)?;
            println!(
            ...
            let manifest = ctcb_dedup::create_deduped_structure(&stripped_dir, &deduped_dir)?;
            println!(
                "Saved: {} ({:.1}%)",
                ctcb_core::format_size(manifest.stats.savings),
            ...
            let archive_name = format!("llvm-{}-{}-{}.tar.zst", version, plat, ar);
            ...
            let parts = if archive_size > 99 * 1024 * 1024 {
                ...
                let split_parts = ctcb_split::split_archive(&archive_path, 95, &output)?;
            ...
            let relative_path = format!("clang/{}/{}/{}", plat, ar, archive_name);
            let href = ctcb_manifest::lfs_media_url(&relative_path, "main");

            let part_refs: Option<Vec<ctcb_manifest::PartRef>> = parts.map(|p| {
                p.iter()
                    .map(|part| {
                        let part_name =
                            part.path.file_name().unwrap().to_string_lossy().to_string();
                        let part_relative = format!("clang/{}/{}/{}", plat, ar, part_name);
                        ctcb_manifest::PartRef {
                            href: ctcb_manifest::lfs_media_url(&part_relative, "main"),
            ...
            let arch_manifest_relative = format!("{}/{}/manifest.json", plat, ar);
        ...
        Commands::Iwyu {
            ...
            let combos =
                discover_platform_arch_dirs(&iwyu_root, platform.as_deref(), arch.as_deref())?;
        ...
        Commands::MingwSysroot {
            ...
            if !skip_download {
                ...
                let archive_path =
                    archive_dir.join(format!("llvm-mingw-{}.tar.xz", llvm_mingw_version));
            ...
            std::fs::create_dir_all(&output)?;
            let archive_name = format!("mingw-sysroot-{}-{}.tar.zst", llvm_mingw_version, ar);
            let archive_path = output.join(&archive_name);
        ...
        Commands::Emscripten {
            ...
            let ar = ctcb_core::Arch::from_str_loose(&arch)?;
            let output = output_dir
                .unwrap_or_else(|| PathBuf::from(format!("assets/emscripten/{}/{}", plat, ar)));

            ctcb_core::print_section(&format!("PACKAGE EMSCRIPTEN for {}-{}", plat, ar));
        ...
        Commands::EmscriptenDocker {
            ...
            ctcb_core::print_section(&format!("PACKAGE EMSCRIPTEN (Docker) for {}-{}", plat, ar));
        ...
        Commands::Nodejs {
            ...
            let ar = ctcb_core::Arch::from_str_loose(&arch)?;
            let output = output_dir
                .unwrap_or_else(|| PathBuf::from(format!("assets/nodejs/{}/{}", plat, ar)));

            ctcb_core::print_section(&format!("PACKAGE NODE.JS {} for {}-{}", version, plat, ar));
            ...
            let (node_os, node_arch, ext) = match (plat, ar) {
                ...
                (ctcb_core::Platform::Linux, ctcb_core::Arch::X86_64) => ("linux", "x64", "tar.xz"),
                (ctcb_core::Platform::Linux, ctcb_core::Arch::Arm64) => {
                    ("linux", "arm64", "tar.xz")
                }
