...
pub mod bench {
    ...
    pub fn run_benchmarks(source_dir: &Path, output_prefix: &str) -> Result<Vec<BenchResult>> {
        ...
        println!();
        println!("{:<15} {:>6} {:>12} {:>8} {:>10}", "Method", "Level", "Size", "Ratio", "Time");
        println!("{}", "-".repeat(55));
        ...
        for level in [1, 6, 9] {
            ...
            println!("{:<15} {:>6} {:>12} {:>7.1}x {:>10}", "gzip", level, ctcb_core::format_size(size), ratio, ctcb_core::format_duration(elapsed));
            results.push(BenchResult { method: "gzip".into(), level, compressed_size: size, ratio, duration: elapsed });
        ...
        for level in [1, 3, 10, 15, 19, 22] {
            ...
            let size = fs::metadata(&output)?.len();
            let ratio = tar_size as f64 / size as f64;
            println!("{:<15} {:>6} {:>12} {:>7.1}x {:>10}", "zstd", level, ctcb_core::format_size(size), ratio, ctcb_core::format_duration(elapsed));
            results.push(BenchResult { method: "zstd".into(), level, compressed_size: size, ratio, duration: elapsed });
            fs::remove_file(&output)?;
        }
