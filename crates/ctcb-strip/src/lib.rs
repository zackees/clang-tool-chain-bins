    ...
    for lib_dir_name in &["lib", "lib64"] {
        ...
        for entry in fs::read_dir(&src_lib)? {
            ...
            if entry.file_type()?.is_dir() {
                ...
                let is_removable = REMOVE_LIB_EXTENSIONS
                    .iter()
...
pub fn strip_llvm(source_dir: &Path, output_dir: &Path, config: &StripConfig) -> Result<StripStats> {
    ...
    println!(
        "Final size:     {:>10}",
        ctcb_core::format_size(final_size)
    );
    println!(
        "Saved:          {:>10} ({:.1}%)",
